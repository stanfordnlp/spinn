...
The full PINN model is shown in Figure~\ref{fig:model:1d}. The PINN takes the form of a transition-based parser, but is designed to produce sentence representations given parse structures, rather than producing parse structures as its output. It extends the standard transition-based model in the following ways:
\begin{itemize}
...
\item After processing a sentence, the model has a single vector representation of the sentence on its stack. This representation is a differentiable function of all prior representations. We can thus learn the weights of the model via backpropagation of some downstream error.
\end{itemize}
