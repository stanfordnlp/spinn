...
The core SPINN model implements a sequential shift-reduce parser which operates over sentences. It is designed to produce a vector representation of the sentence as output,as opposed to a tree as in standard shift-reduce \textit{parsing}. This relies on two key modifications to the shift-reduce algorithm:
\begin{itemize}
...
In the SPINN, each row of the stack and each row of the buffer contains a pair of vectors $\langle \vec{h}, \vec{c}\rangle$, which together represent some word or nonterminal node in the parse tree of the sentence. \todo{[SB] Clarify that this is needed because of the TreeLSTM fn.}
...
\caption{Runtime performance results with the thin-stack GPU algorithm for tree-structured neural networks. \todo{[JG] Add some comments}}
\end{figure}

\todo{[JG] Write me! In short: We're an order of magnitude faster than plain C++ CPU TreeRNNs w/ reasonable model sizes.}
...
We find that:
\begin{itemize}
\item Our SPINN-NP-NT model precisely implements tree-structured compositionality (as in a TreeLSTM), but is capable of exploiting batched computation for dramatic \todo{[JG] Numbers} improvements in inference speed.
