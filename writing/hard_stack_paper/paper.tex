...
Tree-structured neural networks are linguistically principled models for sentence encoding that have proven effective on several natural language understanding tasks. However, these models can only operate on parsed sentences, and do not support batched computation, making them prohibitively slow for large-scale tasks. We present the parser--interpreter neural network, or PINN, which implements the sentence encoding computations of a tree-structured neural network within the structure of a transition-based neural constituency parser. This model addresses both of the weaknesses of existing tree-structured models: the transition-based architecture of the parser makes it possible to perform tree-structured sentence interpretation in an efficient, batched, setting, and the deep integration with the parser allows the model to handle interpret unparsed data at test time with nearly no loss in accuracy. Finally, we show that this architecture supports a novel and effective method for handling local context in sentence interpretation, and integrates will with soft attention methods for sentence pair tasks.
