...
\scalebox{0.6}{
 \begin{tikzpicture}
    ...
    \node[softmax] (0sm) at  (\ox+3.25*\dx,7*\dy) {transition};
    \node[result] (0so) at  (\ox+3.25*\dx,9*\dy) {\reduce};
    ...
    \node[softmax] (1sm) at  (\ox+3.25*\dx,7*\dy) {transition};
    \node[result] (1so) at  (\ox+3.25*\dx,9*\dy) {\shift};
 ...
 \caption{The SPINN unrolled for two transitions during the processing of the sentence \word{the cat sat down}.}\label{fig:model:1d}
...
\end{subfigure}
\caption{\label{m1-views}Two views of the SPINN. In both views, the lower boxes represent the input buffer, and the upper boxes represent the stack. Yellow highlighting indicates which portions of these data structures are visible to the tracking LSTM and to the composition function. Thin gray arrows indicate connections which are blocked by a gating function, and so contribute no information. \todo{[SB] Distinguish functions from data (stack, ops), say that most boxes are NNs. Make sure there is a ref to this figure.}}
\end{figure*}
